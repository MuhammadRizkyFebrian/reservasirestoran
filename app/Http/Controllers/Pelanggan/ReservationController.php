...
class ReservationController extends Controller
{
    ...
    {
        $pemesanan = Pemesanan::with('meja')
            ->where('kode_transaksi', $kode)
            ->where('id_pelanggan', auth('pelanggan')->id())
            ->get();

        return view('pelanggan.payment', compact('pemesanan', 'kode'));
    }
    ...
    public function confirmPayment(Request $request)
    {
        try {
            ...
            $pemesanan = Pemesanan::where('kode_transaksi', $request->kode_transaksi)
                ->where('id_pelanggan', auth('pelanggan')->id())
                ->first();
    ...
    public function cancelReservation($kode)
    {
        try {
            ...
            $pesanan = Pemesanan::where('kode_transaksi', $kode)
                ->where('id_pelanggan', auth('pelanggan')->id())
                ->get();
            ...
            foreach ($pesanan as $p) {
                ...
                Pembayaran::where('id_pemesanan', $p->id_pemesanan)
                    ->update(['status' => 'dibatalkan']);

                // Kembalikan status meja menjadi tersedia
                Meja::where('no_meja', $p->no_meja)
    ...
    {
        $reservasi = Pemesanan::where('id_pelanggan', auth('pelanggan')->id())
            ->select(
                'kode_transaksi',
        ...
        return view('pelanggan.reservation-history', compact('reservasi'));
    }
    ...
    public function uploadPaymentProof(Request $request)
    {
        try {
            ...
            $pesanan = Pemesanan::where('kode_transaksi', $request->kode_transaksi)
                ->where('id_pelanggan', auth('pelanggan')->id())
                ->get();
    ...
    {
        try {
            $pesanan = Pemesanan::with(['meja', 'pembayaran'])
                ->where('kode_transaksi', $kode)
                ->where('id_pelanggan', auth('pelanggan')->id())
                ->first();
            ...
            return view('pelanggan.reservation-receipt', $data);
        } catch (\Exception $e) {
            return redirect()->route('reservation-history')
    ...
    public function checkTableAvailability(Request $request)
    {
        try {
            ...
            $bookedTables = Pemesanan::where(function ($query) use ($datetime, $endDatetime) {
